...
object ApiVersionClassifierUsageChecker : ClassifierUsageChecker {
    override fun check(
        ...
        if (!languageVersionSettings.isAccessToDescriptorAllowed(targetDescriptor)) {
            trace.report(Errors.API_NOT_AVAILABLE.on(element, targetDescriptor.getSinceVersion()!!, languageVersionSettings.apiVersion))
        }
    }
}
