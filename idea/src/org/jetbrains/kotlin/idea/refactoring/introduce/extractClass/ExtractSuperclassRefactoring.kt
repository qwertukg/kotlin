...
class ExtractSuperclassRefactoring(
                ...
                runReadAction {
                    ...
                    for (element in elementsToMove) {
                        ...
                        if (element is KtNamedFunction
                            ...
                            element.toLightMethods().flatMapTo(usages) { MethodReferencesSearch.search(it).map { MoveRenameUsageInfo(it, element) } }
                        }
                    }
    ...
    fun performRefactoring(editor: Editor?) {
        ...
        project.executeWriteCommand(KotlinExtractSuperclassHandler.REFACTORING_NAME) {
            ...
            if (TypeConversionUtil.getMaybeSuperClassSubstitutor(superClass!!, subClass!!, PsiSubstitutor.EMPTY, HashSet()) == null) {
                CommonRefactoringUtil.showErrorHint(project,
                                                    editor,
                                                    "Refactoring is not possible since super class is unresolved in the new context",
            ...
            PullUpProcessor(
                    extractInfo.originalClass.toLightClass(),
                    newClass.toLightClass() ?: return@executeWriteCommand,
